...
async function main () {
  ...
  topicMessageQuery.subscribe(client,
    ...
    (message) => {
      ...
      signatureVerification = verify(contents.hash, contents.signature, contents.pubkey)
      ...
      const sncMsg = {
        ...
        consensus_datetime: new Date(Math.abs(Number((message.consensusTimestamp.seconds) * 1000) +
(Number(message.consensusTimestamp.nanos) / 1000000))).toISOString().replace(/T/, ' ').replace(/\..+/, ''),
        topic_id: topicId,
        running_hash: toHexString(message.runningHash),

        // derived appnet fields
        payload64: payload64,
        payload: payload,
      ...
      axios.get('https://' + instanceName + '.service-now.com//api/now/table/' + tableName,
        ...
        .then(res => {
          ...
          } else { console.log(Date() + 'Skipping sequence number' + sequenceNumber) }
        })
    }
  )
}
